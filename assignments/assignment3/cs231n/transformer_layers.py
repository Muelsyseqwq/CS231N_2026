...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        K = self.key(key)
        V = self.value(value)
        Q = Q.view(N,S,self.n_head,self.head_dim)# N S H head_dim 拆成多个头
        K = K.view(N,T,self.n_head,self.head_dim) 
        V = V.view(N,T,self.n_head,self.head_dim)
        Q = Q.transpose(1,2)#转换成 N,H,S,head_dim 便于torch计算
        K = K.transpose(1,2)
        V = V.transpose(1,2) 

        K_T = K.transpose(2,3)
        scores = (Q @ K_T) / (self.head_dim ** 0.5) #[N,H,S,T] 表示第 n 个样本、第 h 个 head 中，query 位置 i 对 key/value 位置 j 的注意力分数。
        if attn_mask is not None:
            scores = scores.masked_fill(attn_mask == 0,-1e9)
        attn_weights = torch.softmax(scores,dim = -1) #对每个query的T个key赋予权重
        attn_weights = self.attn_drop(attn_weights)
        out = attn_weights @ V #(N,H,S,head_dim)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        num_patch_per_side = H // P
        num_patches = (H // P) * (W // P)
        patch_dim =  C * P * P
        out = x.reshape(N,C,H//P,P,W//P,P).permute(0,2,4,1,3,5)
        out = out.reshape(N,num_patches,C * P * P)
        out  = self.proj(out)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
