...
def get_similarity_no_loop(text_features, image_features):
    ...
    ############################################################################
    image_features = nn.functional.normalize(image_features)
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    for i in range(len(images)):
        ...
        pred_classes.append(class_texts[pred_idx])
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        self.images = images
        self.device = device
        processed_images = [self.clip_preprocess(Image.fromarray(img)).unsqueeze(0) for img in images]
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        text_features = nn.functional.normalize(text_features)
        pred = text_features @ self.image_features.T
...
class DavisDataset:
    def __init__(self):
        # Import tensorflow_datasets lazily so the CLIP parts of this module work
        # without TFDS, and provide a small Windows compatibility shim for TFDS.
        import sys
        import types
        if "resource" not in sys.modules:
            resource = types.ModuleType("resource")
    ...
    def get_sample(self, index):
        ...
        ds_iter = iter(self.tfds.as_numpy(self.davis))
