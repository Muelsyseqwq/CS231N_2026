...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        fixedinput = self.positional_encoding(embed)
        memory = self.visual_projection(features)
        memory = memory.unsqueeze(1)
        mx = torch.ones(T,T)
        mask = torch.tril(mx)
        decoder_out = self.transformer(fixedinput,memory,mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        x_patch_embed = self.patch_embed(x) #(N,num_patches,word_dims)
        pe = self.positional_encoding(x_patch_embed) 
        out = self.transformer(pe) #(N,num_patches,word_dims)
        out = out.mean(dim = 1)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
