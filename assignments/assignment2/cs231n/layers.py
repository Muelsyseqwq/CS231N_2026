...
def batchnorm_backward(dout, cache):
    ...
    dvar = np.sum(dx_hat*(x - mu) *(-1/2) *np.power(var + eps,-3/2),axis = 0)
    dx = dx_hat *(1/np.sqrt(var + eps)) + dvar *2*(x - mu)/x.shape[0] + dmu/x.shape[0]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    # bn_param ={}
    # bn_param['mode'] = 'train'
    # x_hat , cache1 =  batchnorm_forward(x.T,np.ones(x.shape[0]),np.zeros(x.shape[0]),bn_param)
    # x_hat = x_hat.T
    # out = x_hat* gamma + beta
    # x1,mu1,var1,gamma1,eps1 = cache1
    # cache = (cache1,gamma,x_hat)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def layernorm_backward(dout, cache):
    ...
    dvar = np.sum(dx_hat*(x - mu) *(-1/2) *np.power(var + eps,-3/2),axis =1,keepdims= True)
    dx = dx_hat *(1/np.sqrt(var + eps)) + dvar *2*(x - mu)/x.shape[1] + dmu/x.shape[1]


    # cache1,gamma,x_hat = cache
    # d_hat = dout * gamma
    # dx,_,_ = batchnorm_backward_alt(d_hat.T,cache1)
    # dx = dx.T
    # dbeta = np.sum(dout,axis = 0)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N,C,H,W = x.shape
    out,cache1= batchnorm_forward(x.transpose(0,2,3,1).reshape(N*H*W,C),gamma,beta,bn_param)
    out = out.reshape(N,H,W,C)
    out = out.transpose(0,3,1,2)
    cache = cache1,N,C,H,W
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # Your implementation should be very short; ours is less than five lines. #
    ###########################################################################
    cache1,N,C,H,W = cache
    
    dx,dgamma,dbeta = batchnorm_backward(dout.transpose(0,2,3,1).reshape(-1,C),cache1)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    out = gamma * x_hat + beta
    cache = (G, x_hat, x_hat_group, gamma, var, eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # This will be extremely similar to the layer norm implementation.        #
    ###########################################################################
    G, x_hat, x_hat_group, gamma, var, eps = cache
    N, C, H, W = dout.shape
    group_size = (C // G) * H * W
    ...
    dx_group = (inv_std / group_size) * (
        group_size * dx_hat_group
